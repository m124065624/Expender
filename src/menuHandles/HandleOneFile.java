...
public class HandleOneFile
{
	...
	private void handleMethodInvocation(MethodInvocationInfo methodInvocationInfo)
	{
		...
		Utils.putHashSet(AllExpansions.idToFiles, methodInvocationInfo.methodName.id, Var.tempFile);
		AllExpansions.idToMethodName.put(methodInvocationInfo.methodName.id, methodInvocationInfo.methodName);

		Utils.putHashSet(AllExpansions.idToFiles, methodInvocationInfo.methodName.type.id, Var.tempFile);
		...
		for (Argument argument : arguments)
		{
			...
			Utils.putHashSet(AllExpansions.idToFiles, type.id, Var.tempFile);
			...
			{
				Utils.putHashSet(AllExpansions.idToFiles, identifier.id, Var.tempFile);
				...
				if (identifier instanceof Parameter)
				{
					...
					Utils.putHashSet(AllExpansions.idToFiles, parameter.type.id, Var.tempFile);
	...
	private void handleMethodDeclaration(MethodDeclarationInfo methodDeclarationInfo)
	{
		...
		Utils.putHashSet(AllExpansions.idToFiles, methodDeclarationInfo.methodName.id, Var.tempFile);
		AllExpansions.idToMethodName.put(methodDeclarationInfo.methodName.id, methodDeclarationInfo.methodName);

		if (methodDeclarationInfo.methodName.type != null)
		{
			Utils.putHashSet(AllExpansions.idToFiles, methodDeclarationInfo.methodName.type.id, Var.tempFile);
		...
		{
			Utils.putHashSet(AllExpansions.idToFiles, parameters.get(i).id, Var.tempFile);
			AllExpansions.idToParameter.put(parameters.get(i).id, parameters.get(i));

			Utils.putHashSet(AllExpansions.idToFiles, parameters.get(i).type.id, Var.tempFile);
	...
	private void handleExtend(ClassInfo classInfo)
	{
		...
		Utils.putHashSet(AllExpansions.idToFiles, className.id, Var.tempFile);
		AllExpansions.idToClassName.put(className.id, className);

		ArrayList<ClassName> classNames = classInfo.expans;
		for (int i = 0; i < classNames.size(); i++)
		{
			Utils.putHashSet(AllExpansions.idToFiles, classNames.get(i).id, Var.tempFile);
		...
		for (int i = 0; i < fields.size(); i++)
		{
			...
			Utils.putHashSet(AllExpansions.idToFiles, field.id, Var.tempFile);
			AllExpansions.idToField.put(field.id, field);

			Utils.putHashSet(AllExpansions.idToFiles, field.type.id, Var.tempFile);
		...
		for (int i = 0; i < methodNames.size(); i++)
		{
			...
			Utils.putHashSet(AllExpansions.idToFiles, methodName.id, Var.tempFile);
			AllExpansions.idToMethodName.put(methodName.id, methodName);

			if (methodName.type != null)
			{
				Utils.putHashSet(AllExpansions.idToFiles, methodName.type.id, Var.tempFile);
	...
	private void handleAssign(AssignInfo assignInfo)
	{
		...
		for (int i = 0; i < left.size(); i++)
		{
			Utils.putHashSet(AllExpansions.idToFiles, left.get(i).id, Var.tempFile);
		...
		if (right != null)
		{
			for (int i = 0; i < right.size(); i++)
			{
				Utils.putHashSet(AllExpansions.idToFiles, right.get(i).id, Var.tempFile);
